...
page_fault (struct intr_frame *f) 
{
  ...
  /* Brings in the page to which fault_addr refers. */
#ifdef VM
  bool success = false;
  ...
  if (success)
    return;
#endif
  /* If this is caused by kernel and there should not be any data,
     skip this page fault and return,
     so that get_user() will return -1 and syscall will exit. */
  if (!user) {
    f->eip = f->eax;
    f->eax = 0xffffffff;
    return;
