...
thread_start (void)
{
  ...
  intr_enable (); //enabel intr means to enable timer intr which will cause
...
thread_tick (void)
{
  ...
  /* Update statistics. */
  if (t == idle_thread) //because the idle_thread is a static constant pointer, so to check whether current thread is idle_thread, just compare pointer value
    idle_ticks++;
#ifdef USERPROG
  ...
  /* Enforce preemption. */
  //thread_ticks -> # of ticks after last yield
...
thread_preemption (void) {
  if ( !list_empty (&ready_list) ) {
    ...
    if (intr_context()) { 
      //if preemption happens in intrruption duration(espically timer intr)
      thread_ticks++;
      if (thread_current()->priority < max_in_ready->priority 
          || (thread_current()->priority == max_in_ready->priority 
              && thread_ticks >= TIME_SLICE))
        intr_yield_on_return();

    } else if ( thread_current()->priority < max_in_ready->priority )
      thread_yield();
  }
}
...
void print_thread_list( const char *label, const struct list *toPrint ) {
  ...
  struct thread *t;
  printf( "%s:[\n", label );
  for ( e = list_begin(toPrint); e != list_end(toPrint); e = list_next(e) ) {
...
thread_yield (void)
{
  ...
  if (cur != idle_thread) {
    ...
    list_insert_ordered( &ready_list, &cur->elem, thread_compare, &order );
  }
...
{
  //  print_thread_list( "In next_thread_to_run, ready list", &ready_list );
  if (list_empty (&ready_list))
    return idle_thread;
  else
...
allocate_tid (void)
{
  ...
  lock_acquire (&tid_lock);//ensure thread safe
  tid = next_tid++;
...
thread_recalc_recent_cpu (struct thread *t, void *aux) {
  if (t != idle_thread) {
    ...
    //int recent_cpu_f = integer_to_fixedpoint (t->recent_cpu);
